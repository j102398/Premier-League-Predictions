...
def back_to_main_menu_button(row,column):
...
def display_epltable():
  ...
  if team_elements and position_elements and points_elements and matches_played_elements:
    row = 0


    for position, team, points,matchesPlayed in zip(position_elements, team_elements,points_elements,matches_played_elements):
        ...
        outputtedsentencepoints = tk.Label(root,text=(str(points) ))
        outputtedsentencepoints.grid(row=row,column = 4,pady=1)

        outputtedsentencematchesplayed = tk.Label(root,text=str(matchesPlayed))
        outputtedsentencematchesplayed.grid(row=row,column=6,pady=1)

        row += 1
        if row == 21:

          back_to_main_menu_button(10,7)
          break
